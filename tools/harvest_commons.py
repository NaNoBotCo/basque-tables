...
from common import IMAGES as _IM, jdump, jload, node_list as load_nodes  # noqa: E402
from pathlib import Path as _P
IMAGES = _P(_IM)

UA = "basque-tables-build/0.1 (https://nanobotco.github.io/basque-tables; nan@motdang.net) python-urllib"
...
def _members(category: str, ns: int) -> list[str]:
    out, cont = [], {}
    while True:
...
def walk(category: str, depth: int = 0, cap: int = 400) -> list[dict]:
    """Files in a category. Many Commons categories hold only subcategories, so --depth
    walks down into them; cap keeps a broad tree from becoming a download of Commons."""
    ...
    while queue and len(files) < cap:
        ...
        if cat in seen:
            continue
        seen.add(cat)
        for t in _members(cat, 6):
            if t not in files:
                files.append(t)
            if len(files) >= cap:
                break
        if d < depth:
            for sub in _members(cat, 14):
                queue.append((sub, d + 1))
    return imageinfo(files[:cap])
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    if a.walk:
        write_triage(a.walk, walk(a.walk, a.depth, a.cap))
    elif a.search:
        write_triage("search " + a.search, search(a.search, a.limit))
